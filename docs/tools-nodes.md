...
Tous les workflows d'outils normalisent l'entrée (`provider`, `operation`, `params`) via un node Code puis alimentent les champs requis documentés dans `registries/n8n-official-ops/*.json`. Les options facultatives sont regroupées dans `additionalFields` ou `options` selon le node.
...
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), file.download, file.upload, fileFolder.search, folder.create, sampleFetch
...
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), text.classifyTextForViolations, text.generateModelResponse, text.generateChatCompletion, sampleFetch
...
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), conversation.getMany, file.get, file.getMany, file.upload, message.delete, message.getPermalink, message.search, message.send, sampleFetch
...
- Nombre total de nodes : 8
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), message.get, message.getMany, message.send, sampleFetch, sampleFetch utilise send
...
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), event.create, event.getMany, sampleFetch
...
- Nombre total de nodes : 7
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), document.create, document.get, document.update, sampleFetch
...
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), sheet.append, sheet.read, sheet.update, spreadsheet.create, sampleFetch
...
- Nombre total de nodes : 9
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), boardItem.addUpdate, boardItem.create, boardItem.get, boardItem.getMany, boardItem.updateColumnValues, sampleFetch
...
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), company.update, contact.create, sampleFetch
...
- Nombre total de nodes : 5
- Liste : Manual Trigger, Normalize Input (Code), Dispatch Operation (Code), campaign.createCampaign, email.sendEmail, sampleFetch
